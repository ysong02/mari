...
// Single pass over the scan list: update an existing entry, or claim an empty spot, or overwrite the oldest reading, keeping later average-rssi computation cheap.
...
// Compute the average rssi for each gateway and return the highest one; capacity is enforced earlier by simply not adding full gateways to the scan list.
